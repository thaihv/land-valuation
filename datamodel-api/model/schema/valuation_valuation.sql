...
COMMENT ON COLUMN preparation.model_coefficient.value
...
(
    ...
    transaction_id character varying(40) COLLATE pg_catalog."default" DEFAULT uuid_generate_v1(),
    vunit_id character varying(40) COLLATE pg_catalog."default" DEFAULT uuid_generate_v1(),
    surveyed_value numeric(20,2),
    used_in_model_sample boolean NOT NULL DEFAULT false,
    ...
    rowidentifier character varying(40) COLLATE pg_catalog."default" NOT NULL DEFAULT uuid_generate_v1(),
    rowversion integer NOT NULL DEFAULT 0,    
    CONSTRAINT bulk_operation_valuation_pkey PRIMARY KEY (id),
    CONSTRAINT bulk_operation_valuation_transaction_id_fkey FOREIGN KEY (transaction_id)
        REFERENCES transaction.transaction (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT bulk_operation_valuation_vunit_id_fkey FOREIGN KEY (vunit_id)
        REFERENCES valuation.valuation_unit (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
)
...
ALTER TABLE IF EXISTS transaction.bulk_operation_valuation
    OWNER to postgres;

COMMENT ON TABLE transaction.bulk_operation_valuation
    IS 'Used as a staging area when process valuation unit objects with the bulk operations functionality. Data in this table is validated prior to update into the Valuation table.';

COMMENT ON COLUMN transaction.bulk_operation_valuation.id
    IS 'The identifier of the record for valuation unit.';

COMMENT ON COLUMN transaction.bulk_operation_valuation.change_action
    IS 'Indicates if the last data modification action that occurred to the row was insert (i), update (u) or delete (d).';

COMMENT ON COLUMN transaction.bulk_operation_valuation.change_time
    IS 'The date and time the row was last modified.';

COMMENT ON COLUMN transaction.bulk_operation_valuation.change_user
    IS 'The user id of the last person to modify the row.';

COMMENT ON COLUMN transaction.bulk_operation_valuation.rowidentifier
    IS 'Identifies the all change records for the row in the table.';

COMMENT ON COLUMN transaction.bulk_operation_valuation.rowversion
    IS 'Sequential value indicating the number of times this row has been modified.';

COMMENT ON COLUMN transaction.bulk_operation_valuation.assessed_value
    IS 'Value of valuation unit assessed by operation process in numeric.';

COMMENT ON COLUMN transaction.bulk_operation_valuation.surveyed_value
    IS 'Value of valuation unit in investigation or survey in numeric.';

COMMENT ON COLUMN transaction.bulk_operation_valuation.used_in_model_sample
    IS 'Flag used to indicate the value of this valuation unit is used to run model.';

COMMENT ON COLUMN transaction.bulk_operation_valuation.transaction_id
    IS 'The identifier of the transation associated to the bulk operation.';    