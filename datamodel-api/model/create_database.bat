...
REM SET /p pword= DB Password [?] :
SET /p pword= DB Password [%pword%] :
...
REM Extract the test data from the 7z archive and load it into the database if any. 
REM echo Extracting data files...
REM echo ### Extracting data files... >> %BUILD_LOG% 2>&1
REM %zip_exe% e -y -o"%data_path%" "%data_path%data.7z" >> %BUILD_LOG% 2>&1
REM Use -p option if the archive is password protected as follows
REM %zip_exe% e -y -p%archive_password% -o%data_path% %data_path%data.7z >> %BUILD_LOG% 2>&1
