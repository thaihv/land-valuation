...
public class RestExceptionHandlerAdvice {
	...
	public ResponseEntity<Response> responseBizException(BizException ex) {
		...
		return new ResponseEntity<>(new ResponseBuilder<>().fail()
				.error(new ResponseError(ex.getError().getValue(), ex.getMessage(), ex.getError().getDescription()))
				.build(), HttpStatus.INTERNAL_SERVER_ERROR);
	}

	@SuppressWarnings("rawtypes")
	@ExceptionHandler(AccessDeniedException.class)
	public ResponseEntity<Response> responseAccessDeniedException(AccessDeniedException ex) {
		log.error("AccessDeniedException {}", ex.getMessage(), ex);
		return new ResponseEntity<>(new ResponseBuilder<>().fail().error(
				new ResponseError(BizErrorCode.E0004.getValue(), ex.getMessage(), BizErrorCode.E0004.getDescription()))
